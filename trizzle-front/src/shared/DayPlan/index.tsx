        ...
        {placeList.map((dayPlan, index) => (
          <S.DayPlanContainer>
            ...
            {dayPlan.placeList.length > 0 && dayPlan.placeList.map((place:any, index:number) => (
              <Droppable droppableId={`${dayPlan.day}-${index}`} key={index}>
                {(provided) => (
                  <div ref={provided.innerRef} {...provided.droppableProps} style={{width: "100%"}}>
                    <Draggable draggableId={`${dayPlan.day}-${index}`} index={index}>
                      {(provided) => (
            ...
            <Droppable droppableId={`${dayPlan.day}-${dayPlan.placeList.length}`} key={dayPlan.placeList.length}>
              {(provided) => (
                <div ref={provided.innerRef} {...provided.droppableProps} style={{width: "100%"}}>
